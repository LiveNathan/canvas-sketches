...
function animate() {
    ...
    draw.circle(point, radius, "white");

    if (point.x + radius > canvas.width || point.x - radius < 0) velocity.x = -velocity.x; // horizontal walls
    if (point.y + radius > canvas.height || point.y - radius < 0) velocity.y = -velocity.y; // vertical walls
